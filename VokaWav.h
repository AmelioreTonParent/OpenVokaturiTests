#ifndef _header_VokaWav_
#define _header_VokaWav_
/*
	VokaWav.h
	public-domain sample code by Vokaturi, 2018-01-14
	...
	- does not support files with more than "fmt ", "data", "PAD", "LIST", "fact" and "FLLR" chunks
...
inline static void VokaWav_open (const char *fileName, VokaWav *me) {
	my f = fopen (fileName, "rb");
	if (my f == NULL) {
		fprintf (stderr, "VokaWav error: cannot open file %s.\n", fileName);
		return;
	}
	/*
	...
	*/
	...
	{
		fprintf (stderr, "VokaWav error: %s is not a simplistic WAV file.\n", fileName);
		fclose (my f);
		my f = NULL;
		return;
	}
	...
	if (fmtChunkSize != 16) {
		if (fmtChunkSize == 18) {
			...
			*/
			...
			fprintf (stderr, "VokaWav error: %s has a fmt chunk of size %u.\n", fileName, fmtChunkSize);
			fclose (my f);
			my f = NULL;
			return;
		}
	...
	if (audioFormat != 1) {
		fprintf (stderr, "VokaWav error: %s has compressed audio format %u.\n", fileName, audioFormat);
		fclose (my f);
		my f = NULL;
		return;
	}
	...
	if (numberOfChannels > 2) {
		fprintf (stderr, "VokaWav error: %s has more than two channels.\n", fileName);
		fclose (my f);
		my f = NULL;
		return;
	}
	...
	if (sampleRate < 8000) {
		fprintf (stderr, "VokaWav error: a sample rate of %u Hz is not supported.\n", sampleRate);
		fclose (my f);
		my f = NULL;
		return;
	}
	...
	if (bitsPerSample != 16) {
		fprintf (stderr, "VokaWav error: %s is not 16-bit PCM.\n", fileName);
		fclose (my f);
		my f = NULL;
		return;
	}
		...
		! strncmp (header + 36, "FLLR", 4))
	{
		...
		if (chunkSize > 9000) {
			fprintf (stderr, "VokaWav error: %s has a too large \"%.4s\" chunk.\n", fileName, header + 36);
			fclose (my f);
			my f = NULL;
			return;
		}
		if (! strncmp (header + 36 + 4 + 4 + chunkSize, "data", 4)) {
			...
			fprintf (stderr, "VokaWav error: %s has no data chunk.\n", fileName);
			fclose (my f);
			my f = NULL;
			return;
		}
	} else {
		fprintf (stderr, "VokaWav error: %s contains un unrecognized \"%.4s\" chunk.\n", fileName, header + 36);
		fclose (my f);
		my f = NULL;
		return;
	}
	...
	if (dataChunkSize < 100 * bytesPerSample * numberOfChannels) {
		fprintf (stderr, "VokaWav error: %s does not have at least 100 samples.\n", fileName);
		fclose (my f);
		my f = NULL;
		return;
	}
...
inline static int VokaWav_valid (VokaWav *me) {
	return my f != NULL;
}

inline static void VokaWav_close (VokaWav *me) {
	if (my f) fclose (my f);
}

inline static void VokaWav_clear (VokaWav *me) {
	if (my f) {
		fclose (my f);
		my f = NULL;
	}
...
inline static void _vokaWav_readToCache (VokaWav *me,
...
inline static void VokaWav_fillSamples (VokaWav *me,
	int channel,   // 0 = left or only channel; 1 = right channel
	int startSample,   // base-0
	int numberOfSamples,
	double *samples)
{
	_vokaWav_readToCache (me, startSample, numberOfSamples);
...
inline static double * VokaWav_readAll (VokaWav *me, int channel) {
	double *samples = (double *) calloc (my numberOfSamples, sizeof * samples);
	if (samples == NULL) return NULL;
	VokaWav_fillSamples (me, channel, 0, my numberOfSamples, samples);
	return samples;
}

inline static void VokaWav_fillVoice (VokaWav *me, VokaturiVoice voice,
	int channel,   // 0 = left or only channel; 1 = right channel
	int startSample,   // base-0
	int numberOfSamples)
{
	...
	for (int isamp = 0; isamp < numberOfSamples; isamp ++) {
		...
		int16_t sample_int = (int16_t) (uint16_t) (p [0] | p [1] << 8);   // signed little-endian two bytes
		p += 2;
		VokaturiVoice_fill_int16value (voice, sample_int);
		if (my numberOfChannels > 1 && channel == 0) p += 2;   // skip right channel
	}
}

/* End of file VokaWav.h */
#endif
